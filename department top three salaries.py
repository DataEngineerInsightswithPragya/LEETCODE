...
spark = SparkSession.builder.appName("department-top-three-salary").getOrCreate()
...
joined_df = employee_df.join(department_df,employee_df["departmentId"] == department_df["id"],"inner")\
                       .drop(department_df["id"])\
                       .withColumn("Department",department_df["name"])\
                       .drop(department_df["name"])
joined_df.show()

dense_df = joined_df.withColumn("rank_num",dense_rank().over(WindowSpec))
dense_df.show()

result_df = dense_df.where(dense_df["rank_num"] <= 3).select("Department",dense_df["name"].alias("Employee"),dense_df["salary"].alias("Salary"))
result_df.show()
